...
import { getPortalLoginUrl, isLocalSingleHostMode, portalHosts, portalVariant } from './portal/portalConfig';
  ...
  if (!user) {
    if (portalVariant === 'cloud' || portalVariant === 'devops' || portalVariant === 'email' || portalVariant === 'enterprise') {
      if (isLocalSingleHostMode()) {
        window.location.assign(`${portalHosts.home}?target=${portalVariant}`);
        return null;
      }
